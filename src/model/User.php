...
class User{
   ...
   public function newUser($arr){
      ...
      if(self::checkExistUser($iduser, $cpf)){
         $query = "UPDATE `user` SET `time`='$time',`name`='$nome',`sobrenome`='$sobrenome',`cpf` = '$cpf',`password`='$senha',`id_pcu`= $idpcu,`email`='$email',`telPessoal`= '$telpessoal',`telEmpresarial`= $telempresarial WHERE `id_user` = '$iduser'";
         $class = 'danger';
