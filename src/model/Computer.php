...
class Computer{
   ...
   public static function newComputer($arr){
      ...
      $query = "INSERT INTO computer (`marca`,`modelo`,`numserie`,`memoria`,`type`, `hd`,`processador`, `mac` ) VALUES ('$marca','$modelo','$numserie','$memoria', '$typepc', '$hd', '$processador', '$mac')";
      $class = 'success';
      $text = 'Computador Cadastrado!';

      if(self::checkExistComputer($numserie, $mac)){
         $query = "UPDATE `computer` SET `marca`='$marca',`type`='$typepc',`numserie`='$numserie',`hd`='$hd',`processador`='$processador',`modelo`='$modelo',`mac`='$mac',`memoria`='$memoria' WHERE `mac` = '$mac' AND `numserie` = '$numserie'";
         $class = 'danger';
